...
const sentryInstance = new _src_main__WEBPACK_IMPORTED_MODULE_0__["Sentry"]({
  apikey: 'xxx'
});
sentryInstance.installVuePlugin(vue_dist_vue_min__WEBPACK_IMPORTED_MODULE_1___default.a);
...
const createVue = function () {
  const elm = createElm();
  return new vue_dist_vue_min__WEBPACK_IMPORTED_MODULE_1___default.a({
    template: '',
    methods: {
      _click: function () {}
    }
...
class Sentry {
  constructor(options = {}) {
    this.options = options; // apikey特殊处理
    // @TODO 健壮性校验
  ...
  init() {
    ...
    const windowModel = new WindowModel(); // window 模块安装

    commonModel.install(windowModel.errorType, windowModel.valid, windowModel.callback); // http 模块安装

    commonModel.install(httpModel.errorType, httpModel.valid, httpModel.callback); // xhr 模块
...
class CommonModel {
  ...
  addErrorEventListener() {
    const _this = this; // DOM3级以及Opera部分错误类型不会冒泡 需要在捕获阶段获取
  ...
  send(...args) {
    ...
    args = args.map(item => {
      return Object(_utils__WEBPACK_IMPORTED_MODULE_0__["extend"])(item, commonMsg);
    });
    return _utils__WEBPACK_IMPORTED_MODULE_0__["report"].apply(this, args);
  } // 日志打印
  ...
  getCommonReportMessage() {
    const apikey = APIKEY;
    const version = _config__WEBPACK_IMPORTED_MODULE_1__["default"].version;
  ...
  vuePlugin(Vue, callback) {
    ...
    Vue.config.errorHandler = function VueErrorHandler(error, vm, info) {
      ...
      let _errorMessageForReport = {
        type: _config__WEBPACK_IMPORTED_MODULE_1__["default"].typeDict.vue,
        vueMessage,
        componentName: '',
        lifecycleHook: null
      }; // vm 和生命周期钩子不一定总是可用

      if (Object.prototype.toString.call(vm) === '[object Obejct]') {
        _errorMessageForReport.componentName = this.formatComponentName(vm);
      }
...
class WindowModel extends CommonModel {
  ...
  callback(errorEvent) {
    ...
    const _errorMessageForReport = {
      type: _config__WEBPACK_IMPORTED_MODULE_1__["default"].typeDict.window,
...
class HttpModel extends CommonModel {
  ...
  callback(errorEvent) {
    const type = _config__WEBPACK_IMPORTED_MODULE_1__["default"].typeDict.http;
    const path = Object(_utils__WEBPACK_IMPORTED_MODULE_0__["pathFormartter"])(errorEvent.path).reverse().join(' > ');
...
class PromiseModel extends CommonMode {} // xhr模块
...
class XHRModel extends CommonModel {
  ...
  wrap(options, func, _before) {
    const _this = this; // 非函数单参数 返回options本身


    if (Object(_utils__WEBPACK_IMPORTED_MODULE_0__["isUndefined"])(func) && !isFuntion(options)) {
      return options;
    } // 函数单参数 封装函数


    if (Object(_utils__WEBPACK_IMPORTED_MODULE_0__["isFunction"])(options)) {
      func = options;
      options = undefined;
    }

    if (!Object(_utils__WEBPACK_IMPORTED_MODULE_0__["isFunction"])(func)) {
      return func;
    } // 避免二次封装
    ...
    function wrapped() {
      let args = [],
          i = arguments.length,
          // 深层封装 除非制定deep = false
      deep = !options || options && options.deep !== false; // before 钩子函数

      if (_before && Object(_utils__WEBPACK_IMPORTED_MODULE_0__["isFunction"])(_before)) {
      ...
      try {
        ...
        _this._ignoreNextError(); // 手动捕获一个异常并且发送给平台
        // 目前函数内部谢姐比较多 专注捕获业务代码本身
    ...
    for (var property in func) {
      if (Object(_utils__WEBPACK_IMPORTED_MODULE_0__["hasKey"])(func, property)) {
        wrapped[property] = func[property];
      }
    }

    wrapped.prototype = func.prototype; // 将封装函数赋值给 _sentry_wrapper__

    func.__sentry_wrapper__ = wrapped; // 是否封装过多标志位

    wrapped.__sentry__ = true; // 储存原函数

    wrapped.__orig__ = func;
    return wrapped;
  }

  captureErrorRequest(obj) {// @TODO 相应时间
    // const crumb = extend({
    //   timestamp: now() / 1000
    // },
    // obj
...
/*!**********************!*\
  !*** ./src/utils.js ***!
  \**********************/
/*! exports provided: report, polyfill, isFunction, isUndefined, isString, hasKey, pathFormatter, extend */
/***/ (function(module, __webpack_exports__, __webpack_require__) {
...
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "pathFormatter", function() { return pathFormatter; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "extend", function() { return extend; });
/* harmony import */ var _config__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./config */ "./src/config.js");

function report(message) {
  const xhr = new XMLHttpRequest();
  const reportPort = _config__WEBPACK_IMPORTED_MODULE_0__["default"].protocol.indexOf('https') > -1 ? _config__WEBPACK_IMPORTED_MODULE_0__["default"].httpsServerPort : _config__WEBPACK_IMPORTED_MODULE_0__["default"].httpServerPort;
  const reportUrl = `${_config__WEBPACK_IMPORTED_MODULE_0__["default"].protocol}//${_config__WEBPACK_IMPORTED_MODULE_0__["default"].serverIP}:${reportPort}${_config__WEBPACK_IMPORTED_MODULE_0__["default"].path}`;
...
function polyfill(obj, name, replacement) {
  if (obj === null) return;
  const orig = obj[name];
...
//# sourceMappingURL=sentry.v1.0.0.js.map